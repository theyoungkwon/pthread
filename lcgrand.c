...
int uniform(int lower, int upper, int seed)
{
  ...
  zi = (630360016*zi)%MODLUS;
  zrng[seed] = zi;
  retVal = (zi>>7|1)/16777216.0;
  return (int)(lower + (upper + 1 - lower) * retVal);
}

int uniform64bit(int lower, int upper, int seed)
{
  return (int)(lower + (upper + 1 - lower) * lcgrand64bit(seed));
}
...
{
  Element people;
  people.x = (int)(lower + (upper + 1 - lower) * lcgrand64bit(seed_peo_x));
  people.y = (int)(lower + (upper + 1 - lower) * lcgrand64bit(seed_peo_y));
  people.z = (int)(lower + (upper + 1 - lower) * lcgrand64bit(seed_peo_z));
  people.eat = (int)(min_eating + (max_eating + 1 - min_eating) * lcgrand64bit(seed_peo_eat));
  people.consume = (int)(min_consume + (max_consume + 1 - min_consume) * lcgrand64bit(seed_peo_consume));
  people.sugar = (int)(min_sugar + (max_sugar + 1 - min_sugar) * lcgrand64bit(seed_peo_sugar));
  // people.isMoved = 0;
  return people;
}
...
{
  PeopleDirection mapDirection;
  mapDirection.idx_x = (int)(lower + (upper + 1 - lower) * lcgrand(seed_map_x));
  mapDirection.idx_y = (int)(lower + (upper + 1 - lower) * lcgrand(seed_map_y));
  mapDirection.idx_z = (int)(lower + (upper + 1 - lower) * lcgrand(seed_map_z));
  return mapDirection;
}
